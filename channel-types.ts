...
export interface ChannelAdapter {
  ...
  // Audio/TTS (optional - Matrix adapter implements these)
  sendAudio?(chatId: string, text: string): Promise<void>;

  // Reactions (optional - add emoji reactions to messages)
  addReaction?(chatId: string, messageId: string, emoji: string): Promise<void>;

  // Image (optional - get pending image for multimodal)
  getPendingImage?(chatId: string): { imageData: Buffer; format: string } | null;

  // Message tracking (optional - for reaction feedback)
